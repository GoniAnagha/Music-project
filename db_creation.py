...
# === DB Config for Render PostgreSQL ===
# Paste your FULL External Database URL from Render here
DATABASE_URL = "postgresql://music_project_o893_user:WpC4oUjRxETZyolICZN7vtt8XvJT6RWZ@dpg-d4kk27be5dus73f9h6m0-a.oregon-postgres.render.com/music_project_o893"

# === Directory where your audio files are ===
AUDIO_DIR = "audio/"

# Connect to PostgreSQL using the full URL
...
# -----------------------------------------------------------------
cursor.execute("DROP TABLE IF EXISTS melakarta_audio CASCADE")
cursor.execute("DROP TABLE IF EXISTS raagas_audio CASCADE")
cursor.execute("DROP TABLE IF EXISTS swaras CASCADE")
cursor.execute("DROP TABLE IF EXISTS melakartas CASCADE")
...
# Insert melakarthas using execute_values for better performance
...
if os.path.exists(AUDIO_DIR):
    ...
    for file in os.listdir(AUDIO_DIR):
        if file.endswith(".m4a"):
            name = os.path.splitext(file)[0]
            filepath = os.path.join(AUDIO_DIR, file)
            audio_data.append((name, filepath))
            audio_files_inserted += 1
...
# -----------------------------------------------------------------
# 4. Create melakarta_audio table
# -----------------------------------------------------------------
cursor.execute("""
CREATE TABLE melakarta_audio (
    melakartha_number INT PRIMARY KEY,
