...
def chat():
    ...
    try:
        ...
        conn = connect(
            host="localhost",
            user="trishra",              # ⚙️ adjust if needed
            password="ASV@music2025",    # ⚙️ your MySQL password
            database="music_project"
...
def analyze_audio():
    try:
        ...
        for raga_info in filtered_ragas:
            ...
            # Ask chatbot for arohanam and avarohanam
            chatbot_prompt = f"Give arohanam and avarohanam of {raga_name}"
            
            try:
                # Start a fresh conversation for each query
                convo = model.start_chat(history=[])
                response = convo.send_message(chatbot_prompt)
