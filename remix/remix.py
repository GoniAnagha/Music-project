...
# Single raga data - Vageeshwari
raga_name = "Vāgeeshwari"
arohanam = "Sa R1 G1 M1 Pa D1 Sa higher"
avarohanam = "Sa higher D1 M1 Pa G1 R1 Sa"
...
def combine_notes_only(note_files, output_file):
    """Combine note files without background music"""
    combined_notes = AudioSegment.empty()
    ...
    combined_notes.export(output_file, format="mp3")
    print(f"Saved audio (no background): {output_file}")
...
def change_audio_speed(input_file, output_file, speed=1.0):
    ...
    y_stretched = librosa.effects.time_stretch(y, rate=speed)
    sf.write(output_file, y_stretched, sr)
    print(f"Exported with speed adjustment: {output_file}")

def process_vageeshwari(raga_name, arohanam, avarohanam, output_folder, speed=1.5):
    """Process Vageeshwari raga and create separate arohanam and avarohanam audio files"""
    # Clean the raga name
    clean_name = clean_raga_name(raga_name)
    ...
    # Create arohanam file (without background)
    arohanam_temp = os.path.join(output_folder, f"{clean_name}_arohanam_temp.mp3")
    arohanam_final = os.path.join(output_folder, f"{clean_name}_arohana.mp3")
    combine_notes_only(arohanam_files, arohanam_temp)
    change_audio_speed(arohanam_temp, arohanam_final, speed=speed)
    
    # Create avarohanam file (without background)
    avarohanam_temp = os.path.join(output_folder, f"{clean_name}_avarohanam_temp.mp3")
    avarohanam_final = os.path.join(output_folder, f"{clean_name}_avarohana.mp3")
    combine_notes_only(avarohanam_files, avarohanam_temp)
    change_audio_speed(avarohanam_temp, avarohanam_final, speed=speed)
    
    # Clean up temp files
    if os.path.exists(arohanam_temp):
        os.remove(arohanam_temp)
        print(f"Cleaned up: {arohanam_temp}")
    if os.path.exists(avarohanam_temp):
        os.remove(avarohanam_temp)
        print(f"Cleaned up: {avarohanam_temp}")
...
if __name__ == "__main__":
    ...
    output_folder = "vageeshwari_audio"
    ...
    # Process Vageeshwari raga
    try:
        arohanam_file, avarohanam_file = process_vageeshwari(
            raga_name, arohanam, avarohanam, output_folder, speed=1.5
        )
        print(f"\n✓ Successfully created: {arohanam_file}")
        print(f"✓ Successfully created: {avarohanam_file}")
        print(f"\nAll files saved in folder: {output_folder}")
    except Exception as e:
        print(f"\n✗ Error processing {raga_name}: {e}")